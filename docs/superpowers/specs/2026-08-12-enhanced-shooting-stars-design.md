...
The visual direction is a balanced mix: mostly white meteors with subtle gold and lavender variation, richer tails and glow, occasional brighter hero comets, small interaction-driven bursts, and rare ambient meteors whose cadence becomes temporarily more active after transmission interaction.
...
- Temporarily increase ambient meteor frequency after transmission interaction, then decay back to a calmer cadence.
...
The controller also owns the ambient scheduler and recent-transmission-activity state. Ambient scheduling remains page-memory only.
...
- One or two particles per ambient event.
...
- Refreshes the 60-second activity-boost window for ambient scheduling.
...
- Does not refresh the ambient activity-boost window; the boost is specifically tied to transmission interaction.
...
Calm mode uses a randomized interval between 45 and 90 seconds. Revealing either transmission starts or refreshes a 60-second active window. While the active window is in effect, ambient waits use a shorter randomized interval between 20 and 40 seconds. Once 60 seconds pass without another transmission reveal, the next scheduling decision returns to the 45-to-90-second calm range.

The scheduler must avoid a fixed visible cadence and own exactly one ambient timer at a time. Activity changes affect the next scheduling decision rather than spawning extra timers.
...
No animation should be created only to have CSS collapse its duration. The controller avoids scheduling or rendering the effect in the first place.

## Lifecycle and Cleanup

The current fixed two-second cleanup delay will be replaced by timing derived from the rendered specifications.

For each render batch, cleanup occurs after:
...
The shooting-star layer holds one effect batch at a time. Starting a new batch replaces the currently rendered batch, cancels its cleanup timer, renders the new batch, and schedules cleanup for the new batch. This keeps DOM ownership bounded and matches the current replacement behavior rather than introducing overlapping particle queues.

Destroy/cleanup behavior must cancel both the ambient timer and the shooting-star cleanup timer and clear any rendered shooting-star particles.
...
- Anti-Cringe triggering its preset without refreshing the ambient activity boost;
- anniversary entry triggering the anniversary preset;
- calm ambient scheduling in the 45-to-90-second range on initialization;
- transmission interaction refreshing the 60-second activity window;
- active ambient scheduling in the 20-to-40-second range;
- decay back to calm cadence after the activity window;
- only one ambient timer being owned at a time;
- new shooting-star batches replacing old batches rather than accumulating;
...
- destroy cancelling all effect-related timers and clearing the effect layer.
...
- transmission interaction temporarily makes the ambient sky more active and then naturally settles;
